...
@objc public protocol CircularTimerViewDelegate: class {
    @objc optional func timerDidUpdate(newValue: Int)
...
public class CircularTimerView: UIView {
    ...
    private var currentCounterValue: Int = 0 {
        didSet {
            ...
            delegate?.timerDidUpdate?(newValue: currentCounterValue)
        }
    }
    ...
    public func configure(beginingValue: Int, interval: TimeInterval = 1) {
    ...
    public func start(beginingValue: Int, interval: TimeInterval = 1) {
        ...
        timer?.invalidate()
        timer = Timer(timeInterval: fireInterval, target: self, selector: #selector(CircularTimerView.timerFired(_:)), userInfo: nil, repeats: true)
